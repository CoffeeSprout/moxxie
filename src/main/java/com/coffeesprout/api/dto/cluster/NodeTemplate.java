...
public record NodeTemplate(
    ...
    @NotNull(message = "At least one disk is required")
    @Size(min = 1, message = "At least one disk is required")
    @Valid
    List<DiskConfig> disks,
    
    @Schema(description = "Network configurations", required = true)
    @NotNull(message = "At least one network is required")
    @Size(min = 1, message = "At least one network is required")
    @Valid
