...
public record ClusterSpec(
    ...
    @NotNull(message = "Cluster type is required") ClusterType type,

    @Schema(description = "Node group specifications", required = true)
    @NotNull(message = "At least one node group is required") @Size(min = 1, message = "At least one node group is required")
    @Valid
