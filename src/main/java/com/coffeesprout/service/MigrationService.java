...
public class MigrationService {
        ...
        try {
            ...
            if (storageResponse == null || storageResponse.getData() == null) {
                if (migrationConfig.useNamingFallback()) {
                    ...
                    throw ProxmoxException.prerequisiteFailed("determine storage type",
                        "storage query failed and naming fallback is disabled",
                        "Enable naming fallback in migration configuration or check storage connectivity");
                }
            }
    ...
    public MigrationStartInfo initiateMigration(int vmId, MigrationRequest request, @AuthTicket String ticket) {
        ...
        try {
            ...
            throw ProxmoxException.internalError("get VM details for migration", e);
        }
        ...
        if (currentNode.equals(request.targetNode())) {
            throw ProxmoxException.conflict("VM migration",
                String.format("VM %d is already on node %s", vmId, currentNode));
        }
        ...
        try {
            ...
            if (task.getData() == null) {
                ...
                throw ProxmoxException.internalError("No task ID returned from Proxmox migration request");
            }
            ...
            throw ProxmoxException.internalError("start VM migration", e);
        }
    }
    ...
    public MigrationResponse migrateVM(int vmId, MigrationRequest request, @AuthTicket String ticket) {
        ...
        try {
            ...
            try {
                ...
                if (!"OK".equals(taskStatus.exitstatus())) {
                    throw ProxmoxException.internalError("Migration task failed with status: " + taskStatus.exitstatus());
                }
            } catch (Exception e) {
            ...
            if (migratedVm == null) {
                ...
                throw ProxmoxException.notFound("VM", String.valueOf(vmId),
                    "VM not found after migration - may still be completing");
            }

            if (!request.targetNode().equals(migratedVm.node())) {
                migration.markFailed("VM not on target node after migration");
                throw ProxmoxException.internalError(String.format(
                    "VM migration failed - VM %d is on %s instead of target %s",
                    vmId, migratedVm.node(), request.targetNode()));
            }
            ...
            if (wasRunning && !request.allowOfflineMigration()) {
                ...
                throw ProxmoxException.prerequisiteFailed("complete online migration",
                    "online migration failed for running VM",
                    "Set allowOfflineMigration=true to retry with offline migration. Error: " + e.getMessage());
            }
            ...
            migration.markFailed(e.getMessage());
            throw ProxmoxException.internalError("migrate VM", e);
        }
    }
    ...
    private MigrationResponse performOfflineMigration(VMResponse vm, MigrationRequest request,
        ...
        try {
            ...
            if (task.getData() == null) {
                throw ProxmoxException.internalError("No task ID returned from Proxmox for offline migration");
            }
            ...
            if (!"OK".equals(taskStatus.exitstatus())) {
                throw ProxmoxException.internalError("Offline migration task failed with status: " + taskStatus.exitstatus());
            }
            ...
            throw ProxmoxException.internalError("perform offline migration", e);
        }
    }
    ...
    public MigrationPreconditionsResponse checkMigration(int vmId, String targetNode, @AuthTicket String ticket) {
        ...
        try {
            ...
            throw ProxmoxException.internalError("check migration preconditions", e);
        }
    }
